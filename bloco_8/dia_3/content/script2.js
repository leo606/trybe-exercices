...
function morningStudents() {
  return estudantes
    .filter((student) => student.turno === "Manhã")
    .map((student) => `${student.nome} ${student.sobrenome}`);
}

const allStudentsMorning = morningStudents();

// Buscar um estudante pelo nome e retornar a situação dele em cada matéria:

function studentSituation(estudante) {
  const pessoa = estudantes.find((element) => element.nome === estudante);
  
  return pessoa.materias.map((element) => {
    if (element.nota >= 60) {
...
console.log(studentSituation("Natalia"));
